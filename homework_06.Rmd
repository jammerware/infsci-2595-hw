...
laplace_result <- my_laplace(
  ...
  start_guess = c(map_res_01$par[1], map_res_01$par[2]),
  logpost_func = my_cv_logpost,
  my_info = hw06_info
)
```
...
To do this, maybe we can sum the density in the portion of the Laplace approximated distribution that is less than the sample average?

```{r}
sum(dnorm(mean(hw06_df$x), mean = laplace_result$mode[1], sd = laplace_result$mode[2]))
```
...
laplace_mu = laplace_result$mode[1]
laplace_sigma = laplace_result$mode[2]

post_samples <- generate_post_samples(
  mvn_info = list(mu = laplace_mu, sigma = laplace_sigma), 
  num_samples = 1e4)
