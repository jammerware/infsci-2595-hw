...
Based on this description, we know that the MLE for $\mu_0$ is 11. 
...
my_logpost <- function(unknowns, my_info)
{
  ...
  log_lik <- sum(dnorm(my_info$xobs, mean = my_info$mu, sd = my_info$tau_0))
  
  # calculate the log-prior on mu
  log_prior_mu <- dnorm(lik_mu, my_info$mu_0, my_info$tau_0, log = TRUE)
  
  # calculate the log-prior on sigma
  log_prior_sigma <-  dnorm(lik_sigma, my_info$tau_0, log = TRUE)
...
sigma_grid_lwr <- 1
sigma_grid_upr <- pnorm(hw06_info$mu_0, mean = hw06_info$mu_0, sd = hw06_info$tau_0)
```
...
To confirm this, we could plot the second derivative and observe plot's behavior about the mode. If the sign of the second derivative changes from negative to positive, it's a minimum. Otherwise, it's a maximum.
