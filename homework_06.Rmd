...
On a few questions (1d) I asked for help in the class Discord. 
...
Now we'll do the second derivative. $N$ can be pulled out front as a constant:

$$
p''(\mu \mid x, \sigma) = \frac{d}{d\mu} N \left[ \frac{1}{\sigma^2} \times (\bar{x} - \mu) \right]
$$

Distributing terms, we get:

$$
p''(\mu \mid x, \sigma) = \frac{d}{d\mu}N \times \left( \frac{d}{d\mu} \frac{\bar{x}}{\sigma^2} - \frac{d}{d\mu} \frac{\mu}{\sigma^2} \right)
$$

Constants drop out:

$$
p''(\mu \mid x, \sigma) = \frac{N}{\sigma^2} \frac{d}{d\mu} \left(\bar{x} - \mu \right) = -\frac{N}{\sigma^2}
$$
...
Maybe I'd set the second derivative equal to zero and solve for mu?
...
$$
\frac{dg^{-1}(\sigma)}{d\sigma} = exp(\sigma)
$$
...
my_cv_logpost <- function(unknowns, my_info)
{
  ...
  log_deriv_adjust <- exp(lik_sigma) 
...
The guesses both converge independently but do not produce equal parameters, and their Hessian matrices are different. Assuming my implementation is correct, this would suggest that the log-posterior surfaces are not unimodal. I currently don't have the log posterior visualization code working, so I don't know if that's the intended insight or not.
...
laplace_result <- my_laplace(
  start_guess = c(10, 6), # not sure what to start with here
  logpost_func = my_cv_logpost,
  my_info = hw06_info
)
```
...
```{r, solution_05c}
set.seed(202004)
post_samples <- generate_post_samples(mvn_info = list(mu = mu_0, sigma = tau_0), num_samples = 1e4)
...
```{r}
post_samples %>%
  ggplot(mapping = aes(y = mu)) + 
  geom_histogram(bins = 55, color = "red", opacity = 0.5) + 
  geom_histogram(bins = 55, color = "blue")
```
