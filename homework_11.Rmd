...
Yes, some of the inputs have very different scales. Low and high-numbered inputs seem to range from 0 to around .05, while some inputs in the middle seem to range from 0 to 1. Some of the inputs seem to have high numbers of outliers as well. 
...
Yes. Because the inputs have very different distributions, preprocessing seems appropriate. I would start with centering and scaling. 
...
Yes, some inputs do seem correlated, as evidenced by the brighter colors around the main diagonal. 
...
The levels seem close enough that I don't believe we should be worried about an imbalance.
...
For this model, my ROC is about **0.769**.
...
The final ROC of the elasticnet model is **0.8567839**.

##### Based on the training results, does the elastic net model favor LASSO or RIDGE more?

The value of `alpha` is low (0.1) in the best tune, suggesting that the model favors Ridge regression over LASSO regression.

##### Should you consider applying preprocessing with the regularized model?

Yes, I would assume that centering and scaling is important for regularized models, because if unscaled, regularization would have different amounts of impact depending on the scale of the input.
...
The best tune for this model has an ROC of **0.8859871**, which is better than the elasticnet's ROC of **0.8567839**.

##### Should you preprocess the inputs before training the neural network?

Yes, my guess is that centering/scaling is important for neural nets because it would affect the scales of the local maxima and affect how gradient descent occurs.
...
The top four features are `V50`, `V57`, `V31`, and `V22`.
...
```{r}
viz_input_grid <- make_test_input_grid(
  all_input_names = my_input_names,
  top_4_inputs = my_top_ranked_inputs,
  all_data = Sonar
)
...
They're somewhat imilar, but the neural net seems to predict more observations of type `M` from low values of `V57`. The elasticnet model predicts more `M` ourcomes overall.
...
For each observation, the probability of each class is included in the grid. The probability of `M` can be selected directly or computed by subtracting the probability of `R` from 1 (since this is a two-class problem).
...
Because forest models don't have coefficients and only are concerned with information gained at a split point, it doesn't really matter where the split point is. Thus, for interpretability, I omit scaling. 
...
The selected value for `mtry` was 2. I suspect this is because a small number of variables are most influential on the outcome.
...
No, and the top 2 parameters for the neural net don't even appear in either of the random forest or XGBoost model's top 20. 
...
```{r, solution_04d}
viz_input_grid_rf <- make_test_input_grid(
  all_input_names = my_input_names,
  top_4_inputs = c("V9", "V10", "V11", "V12"),
  all_data = Sonar
)
```
...
Note that these figures look kind of wonky because some features have more distinct values than others.
...
The RF model looks pretty good. I guess we'd have to take into account that this is a single 5-fold CV, but it has nearly the best ROC with less variability, highest average sensitivity, and decent specificity against the other models. 
...
Yes, by ROC, the RF model is strongest.
