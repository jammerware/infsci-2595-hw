...
```{r, solution_01f}
mod05 <- lm(
  y ~ splines::ns(df$x1, df = 12) * (x2 + I(x2^4)),
  data = df
)
```
...
```{r, solution_01g}
mod06 <- lm(
  y ~ (splines::ns(df$x1, df = 12)) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)),
  data = df
)
```
...
```{r, solution_03d_a}
viz_post_coefs(laplace_03_weak$mode, diag(laplace_03_weak$var_matrix), c(colnames(X03),"whatever this is"))
```


```{r, solution_03d_b}
viz_post_coefs(laplace_06_weak$mode, diag(laplace_06_weak$var_matrix), c(colnames(X06), "whatever this is"))
```
...
info_06_strong <- list(
  yobs = df$y,
  design_matrix = model.matrix(y ~ (splines::ns(df$x1, df = 12)) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)), data = df),
  mu_beta = 0,
  tau_beta = 1,
  sigma_rate = 1
)
