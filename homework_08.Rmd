...
df %>%
  ...
  ggplot() + 
  geom_point(mapping = aes(x = y, y = value)) +
  facet_wrap(~ name)
```

##### Do you think there are trends between either input and the response?

Based on this visualization, there does not seem to be a particularly clear trend between either input and the response.
...
It's hard to tell, but it looks like no coefficient is significant (x1 is close, maybe).
...
```
There are four terms in the model - the intercept term, the two inputs, and a term expressing the interaction between the inputs. `x1` appears significant, as does the interaction between `x1` and `x2`.
...
Saving the formula, because this model is important later.

```{r}
formula03 <- y ~ (x1 + I(x1^2)) * (x2 + I(x2^2))
```
...
This model contains 9 terms (the intercept, 2 linear terms, 2 quadratic terms, and interactions between each pair of linear/quadratic terms). Most features are not significant, but the intercept and `x2` quadratic term are.
...
```{r, solution_01e}
mod04 <- lm(
  y ~ (x1 + I(x1^2) + I(x1^3) + I(x1^4)) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)),
  data = df
)
```
...
```
It appears that in this model, only the intercept is significant.
...
```{r, solution_01f}
mod05 <- lm(
  y ~ splines::ns(x1, df = 12) * (x2 + I(x2^2)),
  data = df
)
```
...
```
In this model, it appears that **no** features are statistically significant.
...
```{r}
formula06 <- y ~ (splines::ns(x1, df = 12)) * (x2 + I(x2^2) + I(x2^3) + I(x2^4))
```
...
```
Based on this visualization, it seems as though **no** features are statistically significant.
...
models %>%
  ...
  mutate(model.name = paste("model", 1:6), .before = r.squared)
```

##### Discussion

According to R-Squared, **model 6** is the best. According to both AIC and BIC, **model 3** is best.
...
I created a function so I don't have to copy and paste the visualization code 6 times:

```{r}
visualize_predictions <- function(model, title) {
  model %>%
...
```{r}
visualize_predictions(pred_lm_01, "Model 1")
```

```{r}
visualize_predictions(pred_lm_02, "Model 2")
```

```{r}
visualize_predictions(pred_lm_03, "Model 3")
```

```{r}
visualize_predictions(pred_lm_04, "Model 4")
```

```{r}
visualize_predictions(pred_lm_05, "Model 5")
```

```{r}
visualize_predictions(pred_lm_06, "Model 6")
```
...
AIC and BIC both rank Model 1 as the worst model. Model 1's prediction and confidence intervals are virtually the same.
...
my_laplace <- function(start_guess, logpost_func, ...)
{
  ...
  list(
    ...
    log_evidence = int,
    converge = ifelse(fit$convergence == 0, "YES", "NO"),
...
```{r, solution_03c}
laplace_03_weak <- my_laplace(rep(0, 10), lm_logpost, my_info = info_03_weak)
paste("Converge:", laplace_03_weak$converge)
```

##### Model 6

```{r}
laplace_06_weak <- my_laplace(rep(0, 66), lm_logpost, my_info = info_06_weak)
paste("Converge:", laplace_06_weak$converge)
```
...
I ended up wrapping `viz_post_coefs` to manage removal of `varphi`.

```{r}
viz_post_coefs_no_varphi <- function(laplace_result, design_matrix) {
  viz_post_coefs(
    post_means = laplace_result$mode[1:ncol(design_matrix)],
    post_sds = sqrt(diag(laplace_result$var_matrix))[1:ncol(design_matrix)],
...
```{r, solution_03d_a}
viz_post_coefs_no_varphi(laplace_03_weak, X03)
```


```{r, solution_03d_b}
viz_post_coefs_no_varphi(laplace_06_weak, X06)
```
...
Quick function, because I know we're doing this again later:

```{r}
calc_bayes_factor <- function(model0, model1) {
  return(
    exp(model0$log_evidence) / exp(model1$log_evidence)
  )
}
```
...
```{r, solution_03e}
calc_bayes_factor(laplace_03_weak, laplace_06_weak)
```

By the Bayes Factor, we see that **model 3** is significantly more likely than model 6.
...
```{r, solution_03f_b}
laplace_03_strong <- my_laplace(rep(0, 10), lm_logpost, my_info = info_03_strong)
laplace_03_strong$converge
```

```{r}
laplace_06_strong <- my_laplace(rep(0, 66), lm_logpost, my_info = info_06_strong)
...
```{r, solution_03g}
viz_post_coefs_no_varphi(laplace_03_strong, X03)
```

##### Model 6

```{r, solution_03g_ii}
viz_post_coefs_no_varphi(laplace_06_strong, X06)
```
...
```{r, solution_03h_b}
laplace_03_very_strong <- my_laplace(rep(0, ncol(X03) + 1), logpost_func = lm_logpost, my_info = info_03_very_strong)
laplace_03_very_strong$converge
```

```{r}
laplace_06_very_strong <- my_laplace(rep(0, ncol(X06) + 1), logpost_func = lm_logpost, my_info = info_06_very_strong)
...
```{r, solution_03i}
viz_post_coefs_no_varphi(laplace_03_very_strong, X03)
```

##### Model 6

```{r}
viz_post_coefs_no_varphi(laplace_06_very_strong, X06)
```
...
A stronger prior belief about the standard deviation results in lower standard deviations in the coefficient posterior distributions.
...
```{r, solution_03k}
calc_bayes_factor(laplace_03_strong, laplace_06_strong)
```

```{r}
calc_bayes_factor(laplace_03_very_strong, laplace_06_very_strong)
```

As the strength of the prior increases, model 6 appears to become more likely.
...
summarize_lm_pred_from_laplace <- function(mvn_result, Xtest, num_samples)
{
  ...
  mu_avg <- colMeans(post)
  y_avg <- mean(pred_test)
...
*My solution here doesn't work because I couldn't figure out how to "create the test design matrix based on the visualization grid". In the past, when we've used `model.matrix`, we specify a complete formula and a dataframe. We can't do that here because the visualization grid has only predictors, not the outcome. I tried generating the predictions using the model, but the matrix dimensions were incorrect. I also tried creating the model matrix without using the y term, but the matrix dimensionality seemed wrong.*

```{r, solution_04c, eval=FALSE}
X03_test <- model.matrix(~ (x1 + I(x1^2)) * (x2 + I(x2^2)), viz_grid)

post_pred_summary_viz_03_weak <- summarize_lm_pred_from_laplace(laplace_03_weak, X03, 5000)
...
*I have no idea why this isn't working, and I'm exhausted.*

```{r, solution_04e, eval=FALSE}
knots <- make_splines_training_knots(12, df, "x1")
X06_test <- model.matrix(~ splines::ns(x1, knots = knots) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)), data = df)
...
```{r, solution_05b}
lasso_03 <- glmnet(X03, df$y)
lasso_06 <- glmnet(X06, df$y)
```
...
##### Which features have not been turned off by the Lasso penalty?
...
```
Model 3 kept its quadratic term for `x2`.
...
Model 6 kept the same feature as Model 3 - the quadratic term on `x2`.
