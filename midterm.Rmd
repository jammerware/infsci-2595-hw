...
I'm creating a function that adds a `geom_point` to represent the MLE, because this comes up a few times over the midterm.

```{r}
plot_post_mle <- function(anew, bnew, ...) {
...
summary_post_df_focus_from_vague %>%
  ...
  plot_post_mle(anew, bnew, position = position_dodge(0.2))
```
...
Yes, they are. This suggests that the posterior likelihood is unimodal. (Mine don't converge to the same thing because my `my_beta_loglik` is incorrect, but that's the best I can do.)
...
Note that this is incorrect because my `my_beta_loglik` function is incorrect, but the backtransformation process should be right.

```{r}
ab_emp_bayes <- exp(log_ab_res_02$par)
```

Instead, I'm assigning the given values from **3g**.
...
I'm assuming the shape parameters from 3g to answer this question, because my estimated ones are wrong. Assuming $a = 13, b = 8$, it represents **21 a priori trials**.
...
The pooled estimate is the estimation of the unknown average for all players, which appears to be a normal distribution as long as players have a certain number of targets. Therefore, asking about players with catch rate greater than .67 corresponds to players who have a catch rate at least one standard deviation above the mean.
