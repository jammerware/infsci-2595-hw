...
Dropping terms on which $\mu$ does not depend:

$$
... \propto \sum_{j=1}^J \left[ m \cdot \log(\mu) + (N - m) \cdot \log(1-\mu) \right] + (a-1) \cdot \mu + (b-1) \cdot(1-\mu) 
$$

Distributing the sum, pull logs out front:
...
summary_post_df_all_empbayes <- post_df_all_empbayes %>%
  mutate(
    post_avg = anew / (anew + bnew),
    post_q05 = dbeta(.05, anew, bnew),
    post_q95 = dbeta(.95, anew, bnew),
