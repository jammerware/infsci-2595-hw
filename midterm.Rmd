...
Take the log of both sides (making the multiplication of terms addition instead):

$$
log(p( \mu \mid (m, N)_j)^J_{j=1}) \propto log \left[ \prod_{j=1}^{J} \left( \binom{N}{m} \mu^m (1-\mu)^{N-m} \right) \right] + \log \left[ \mu^{a-1} (1-\mu) ^ {b-1} \right]
$$

Simplifying log and distributing summation:

$$
... \propto  \sum_{j=1}^J m_j \cdot \log(\mu) + \sum_{j=1}^J(N_j - m_j) \cdot \log(1-\mu) + (a-1) \cdot \log(\mu) + (b-1) \cdot \log(1-\mu) 
$$

Drop log terms, because I don't know what to do with them:

$$
... \propto  \sum_{j=1}^J m_j + \sum_{j=1}^J(N_j - m_j) + (a-1) + (b-1) 
$$
...
Rearranging the definitions of $\bar{m}$ and $\bar{N}$ to solve for the summation series, we get $\bar{m}J = \sum_{j=1}^{J} \left( m_j \right)$ and $\bar{N}J = \sum_{j=1}^{J} \left( N_j \right)$, respectively. Substituting these into what we obtained in **2a**, we get:

$$
\bar{m}J + \bar{N}J - \bar{m}J + (a - 1) \log(\mu) + (b - 1) \log(\mu - 1)
$$
...
It has a beta distribution. My expression in 2c is likely incorrect, but I can sort of see the beta distribution covered in homework 4 there, and the prior was a beta, so the posterior must be a beta.
...
summary_post_df_focus_pooled %>% head
```
...
my_beta_loglik <- function(unknowns, my_info)
{
  ...
  log_lik <- sum(dbeta(my_info$theta, a, b))
...
All players have a high number of targets. The fewest number targets among these 10 players is around 120.
...
```{r, solution_05c}
post_player_eval_top10$num_trials %>% min()
```
