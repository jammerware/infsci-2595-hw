...
prob_high_noise <- readr::read_csv(high_noise_github_file, col_names = TRUE)
```
...
If I understand the `trainControl` method correctly, the model will be trained and tested 5 times, each time with a different fifth of the data held out for testing (5-fold cross validation).
...
Accuracy, kappa, and ROC are not appropriate to a continuous regression problem (since they would require thresholding to use). This leaves us with r-squared, MAE, and RMSE. Without more context, I'm not sure there's a clear best choice, but I chose RMSE under the assumption that we don't need to interpret the error in terms of the original units.
...
```{r, solution_01f_a}
dotplot(high_noise_results, prob_high_noise, metric='RMSE')
```

```{r, solution_01f_b}
dotplot(high_noise_results, prob_high_noise, metric='MAE')
```

Is there a best model?  

Among the models I tested, the fourth, fifth, and sixth-order polynomials have strongest performance, and all seem pretty close to one another by RMSE and MAE. They have the lowest average value and relatively low variability for both metrics. Under the assumption that lower-order models are better if they're within one SE of each other, I'd choose the 4th-order model.
...
In general, it seems that the higher-order coefficients are either barely significant or not significant at all, suggesting that a lower-order model may be better.
...
The relationship between x and y appears significantly less "noisy" than the data in problem 1 - the points are clustered more closely.
...
The 5th-order polynomial seems like the best model, minimizing the value of and variability in both RMSE and MAE. The 6th-order model scores well in both metrics, but I'm arguing for the 5th-order one on the principle that the simpler model will generalize better if it's within one standard error of the more complex one.

The 4th-order model we identified in problem 1 performs much worse on the more noisy dataset.

Additionally, lower-order polynomials performed much worse on the less-noisy dataset.
...
In problem 1, the results suggested that a lower-order polynomial was the best model. We saw that the 4th-order model performed the best but that its lowest-order coefficients were the most significant. In problem 2, the same model performed much worse as a result of the noise, and the 5th-order model, which performed decently in the first problem, outperformed the 4th-order model in the second. My intuition would be that higher-order models perform better in the presence of noise.
...
I couldn't figure out how the `mean()` function would apply here. Is this referring to the base `mean()` function in R (e.g. the arithmetic mean)? I just calculated the proportion
of events and non-events.

```{r, solution_03b}
event_count <- model_pred_df %>% 
  filter(obs_class == "event") %>%
  count
event_count / nrow(model_pred_df)
```

Is the data set balanced?  

The data set is roughly balanced, with 52.8% of cases representing the `event` class.
...
Yes, it seems to. In the outcome variable, most of the non-events are clustered around values of x between -1 and 1, and the lowest probability values also appear in this region.
...
I ended up making Accuracy a function since we also need it in problem 5

```{r, solution_4b}
get_accuracy <- function(data) {
  total_count <- nrow(data)
  nrow(data %>% filter(obs_class == pred_class)) / total_count
}

get_accuracy(model_class_0.5)
```
This means our model predicts the correct outcome in 70.4% of cases.
...
I wrote some functions to extract confusion matrix entries and calculate sensitivity/FPR for this and later problems:

```{r, solution_04d}
get_confusion_count <- function(., pred, obs) {
  (.) %>%
    filter(pred_class == pred, obs_class == obs) %>%
    slice_head() %>%
    pull(n)
}

get_sensitivity <- function(data) {
  confusion <- data %>% count(pred_class, obs_class)
  tp = get_confusion_count(confusion, 'event', 'event')
  fn = get_confusion_count(confusion, 'non_event', 'event')
  tp / (tp + fn)
}

get_fpr <- function(data) {
  confusion <- data %>% count(pred_class, obs_class)
  fp = get_confusion_count(confusion, 'event', 'non_event')
  tn = get_confusion_count(confusion, 'non_event', 'non_event')
  fp / (fp + tn)
}
```
...
```{r}
get_sensitivity(model_class_0.5)
```

##### FPR

```{r}
get_fpr(model_class_0.5)
```
...
Note that this code calls a function I wrote in my solution to **4b**.

```{r, solution_05a}
get_accuracy(model_class_0.7)
```
...
A quick function to deal with specificity:

```{r, solution_05b}
get_specificity <- function(data) {
  confusion <- data %>% count(pred_class, obs_class)
  tn = get_confusion_count(confusion, 'non_event', 'non_event')
  fp = get_confusion_count(confusion, 'event', 'non_event')
  tn / (tn + fp)
...
```{r}
get_sensitivity(model_class_0.7)
```

##### Specificity
```{r}
get_specificity(model_class_0.7)
```
...
##### Sensitivity
```{r}
get_sensitivity(model_class_0.3)
```

##### Specificity
```{r}
get_specificity(model_class_0.3)
```
...
df_roc = tibble(
  ...
  fpr = c(
    get_fpr(model_class_0.3),
    get_fpr(model_class_0.5),
    get_fpr(model_class_0.7)
  )
)
...
df_roc %>%
  ...
  geom_point(mapping = aes(x = fpr, y=sensitivity, color = factor(threshold))) 
```
