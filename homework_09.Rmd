...
summarize_logistic_pred_from_laplace <- function(mvn_result, Xtest, num_samples)
{
  ...
  betas <- post_logistic_pred_samples(Xtest, mvn_result$mode)
  
  # data type conversion
  betas <- as.matrix(betas)
  
  # make posterior predictions on the test set
  pred_test <- Xtest %*% betas
...
Based on RMSE, setting `alpha` to 1 and `lambda` to `r enet_default$bestTune %>% select(lambda)` is best. This implies a stronger use of Ridge regression than lasso.
...
Based on the best tune, the final model uses more Lasso than Ridge regression (because alpha is very small).
