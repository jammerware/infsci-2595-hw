...
$$
y_n \mid \mu_n ~ \text{Bernoulli}(y_n | \mu_n)
$$
...
$$
p(\boldsymbol{\beta}) = \prod_{d=0}^{D+1}
$$
...
train_df %>%
  ggplot(mapping = aes(x = x, y = y)) + 
  geom_jitter(size = 4, alpha = .3, height = .02, width = 0)
...
Overall, the event probability seems to increase as `x` increases.
...
Xmat_cubic <- model.matrix(y ~ poly(x, 3), data = train_df)
Xmat_spline7 <- model.matrix(y ~ splines::ns(x, df = 7), data = train_df)
```
...
```{r, solution_01e_a}
laplace_linear <- my_laplace(rep(0, 2), logpost_func = logistic_logpost, my_info = info_linear)
laplace_cubic <- my_laplace(rep(0, 4), logpost_func = logistic_logpost, my_info = info_cubic)
laplace_spline7 <- my_laplace(rep(0, 8), logpost_func = logistic_logpost, my_info = info_spline7)
```
...
```{r, solution_01e_b}
linear_sd <- laplace_linear$var_matrix %>% diag() %>% sqrt()
linear_upr <- laplace_linear$mode + 2 * linear_sd
linear_lwr <- laplace_linear$mode - 2 * linear_sd

print("BETA0")
print(paste("lower bound", linear_lwr[1]))
print(paste("upper bound", linear_upr[1]))
print("")
print("BETA1")
print(paste("lower bound", linear_lwr[2]))
print(paste("upper bound", linear_upr[2]))
```


###### Coefficient significance

Both coefficients are significant at 95%.
...
```{r, solution_01e_c}
cubic_sd <- laplace_cubic$var_matrix %>% diag() %>% sqrt()
cubic_upr <- laplace_cubic$mode + 2 * cubic_sd
cubic_lwr <- laplace_cubic$mode - 2 * cubic_sd

seq(1:length(laplace_cubic$mode)) %>%
  purrr::map(function(x) {
    print(paste("BETA", x - 1))
    print(paste("lower bound", cubic_lwr[x]))
    print(paste("upper bound", cubic_upr[x]))
  })
```
###### Coefficient significance

At 95%, $\beta_2$ is not significant, but the rest are.
...
```{r, solution_01f}
log_evidence <- purrr::map_dbl(list(
  laplace_linear,
  laplace_cubic,
  laplace_spline7
), "log_evidence")
weights <- exp(log_evidence)
weights
```
##### Which is best?
...
The linear model is better than the cubic model.

```{r}
weights[1] / weights[3]
```

The linear model also beats the spline model. Therefore **the linear model** is most likely.

### 1g)

**Is the log-Evidence identified best model consistent with your initial interpretations from Problem 1b)?**  

#### SOLUTION

Yes. Because 1b showed linear separability, it seems reasonable that a linear model would perform the best.
...
Prior versions of this function involve a backtransformation from log space, while this one doesn't.
...
summarize_logistic_pred_from_laplace <- function(mvn_result, Xtest, num_samples)
{
  ...
  pred_test <- post_logistic_pred_samples(Xtest, t(betas))
  ...
  mu_avg <- rowMeans(pred_test$eta_mat)
...
```{r, solution_2e}
post_pred_summary_linear_class <- post_pred_summary_linear %>%
  mutate(pred_class = ifelse(mu_avg > 0.5, 1, 0))

post_pred_summary_cubic_class <- post_pred_summary_cubic %>%
  mutate(pred_class = ifelse(mu_avg > 0.5, 1, 0))

post_pred_summary_spline7_class <- post_pred_summary_spline7 %>%
  mutate(pred_class = ifelse(mu_avg > 0.5, 1, 0))
```
...
I'm using Caret because I'm tired.
...
```{r}
nrow(subset(post_pred_summary_linear_class_b, pred_class == y)) / nrow(post_pred_summary_linear_class_b)
```

##### Accuracy: cubic

```{r}
nrow(subset(post_pred_summary_cubic_class_b, pred_class == y)) / nrow(post_pred_summary_cubic_class_b)
```

##### Accuracy: splines

```{r}
nrow(subset(post_pred_summary_spline7_class_b, pred_class == y)) / nrow(post_pred_summary_spline7_class_b)
```
...
Yes, I would expect it to struggle. While there's clear linear separability, we don't have sufficient resolution to know exactly where the separator should be (other than between two of the data points)
...
info_small_inform <- list(
  yobs = train_small$y,
  design_matrix = model.matrix(y ~ x, data = train_small),
  mu_beta = 0,
  tau_beta = 2.5
)
...
```{r, solution_03c_a}
small_laplace_from_inform <- my_laplace(c(0, 0), logistic_logpost, my_info = info_small_inform)
small_laplace_from_inform$mode
```

```{r, solution_03c_b}
small_laplace_from_weak <- my_laplace(c(0, 0), logistic_logpost, my_info = info_small_weak)
small_laplace_from_weak$mode
```

##### How do the posterior modes compare? Include text and code chunks to answer this question.  

The informative prior strongly constrains the coefficient size (under 2, with a prior SD of 2.5) while the weak prior does not (due to its prior of 50). This represents the logistic regression attempting to learn a steep-sloped step change, while the strong prior regularizes the coefficients to be more moderate.
...
With the weak prior, the coefficients are highly correlated (almost 1!) With the stronger prior, the correlation is significantly reduced.
...
The GLM fails to converge because it is able to perfectly linearly separate the outputs (i.e., for all values of X less than or equal to -.785, the outcome is predicted to be 1, else 0).
...
Some inputs (e.g. x1, x3) have a clearer trend than the others (x4, for example).
...
```{r, solution_04c1}
correlation_matrix <- df %>% select(-y) %>% cor()
corrplot::corrplot(correlation_matrix)
```


```{r, solution_04c2}
corrplot::corrplot(correlation_matrix, type = "upper")
```

##### Based on your visualizations, are the inputs correlated?

Yes, there are clear correlations (both positive and negative) between inputs.
...
```{r, solution_04d}
model_4d <- lm(y ~ ., data = df)
correlation_4d <- model_4d %>%
  vcov %>%
  cov2cor

corrplot::corrplot(correlation_4d, type = "upper")
```

##### How does the $\beta$ parameter posterior correlation matrix compare to the input correlation matrix?

Fewer of the beta parameters are correlated than the input features.
...
```{r, solution_04f}
model_4f <- lm(y ~ .^3, data = df)
correlation_4f <- model_4f %>%
  vcov %>%
  cov2cor

corrplot::corrplot(correlation_4f, type = "upper")
```
...
Based on RMSE, setting `alpha` to 1 and `lambda` to `r enet_default$bestTune %>% select(lambda)` is best. This implies complete use of LASSO regression (because alpha is 1).
...
##### How many tuning parameter combinations are you trying out?

We're trying **`r 9 * 25`** combinations (9 x 25). Using 5-fold 5-repeat CV, we'll be fitting `r  9 * 25 * 25` models.
...
Based on the best tune, the final model uses more LASSO than Ridge regression (because alpha is nearly 1).
...
Most linear terms are nonzero, and a handful of bivariate interaction coefficients are also nonzero. Only one triplet interaction is nonzero (x1, x2, and x4).
...
##### Ranking comparison with 5e

Yes, these variable importances seem highly related to the coefficient magnitudes observed in **5e**.
