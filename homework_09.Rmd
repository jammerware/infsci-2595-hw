...
```
(Strong prior prevents extreme slopes, )
...
Yes. The informative prior strongly constrains the coefficient size (under 2, with a prior SD of 2.5) while the weak prior does not (due to its prior of 50).
...
enet_grid <- expand.grid(
  ...
  lambda = seq(length.out = 25) # make the grid in log space
)
```
...
Based on the best tune, the final model uses more Ridge than Lasso regression (because alpha is very small).
