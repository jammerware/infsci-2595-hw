...
The boxplot suggests a few things:

- The central tendency of the price variable increases as color increases (where we treat color as an ordinal increasing from D to J). 
- Lower levels of color have a greater number of outliers, which implies that price can fluctuate dramatically at lower color ordinals.
...
diamonds %>%
  ...
  ggplot(mapping = aes(x = color, y = price)) + 
  ggtitle("color/price correlation for diamonds with ideal cut and IF clarity") + 
...
All of these results apply only to gems with ideal cut and clarity value IF.

New insights gained from this figure:

- As color increases, a greater proportion of the diamonds have a carat value greater than the global median.
- Diamonds of color D seem to have two highly localized median values (one for gems below the median carat value and one for gems above it)
- The median price of gems above the global median carat value actually decreases with color (in contrast to what we saw in 2b).
- The interquartile range of the price variable increases with color, and the proportion of diamonds which sell for a price greater than the median largely increases with color.
...
Yes, `displ` is significant because its 95% confidence interval does not include 0.
...
Based on the plot above, all variables are significant. Three coefficients are displayed (`drvr`, `drvf`, and `displ`). 
