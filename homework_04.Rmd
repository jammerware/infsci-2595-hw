...
$$ 
\log \left( p \left( \mu \mid a, b \right) \right) \propto (a - 1) \cdot \log(\mu) + (b - 1) \cdot \log(1 - \mu)
$$
...
I'm starting from here because the problem tells us to add 3a and 3b together. I'm not sure the left side of the equation is correct.

$$ 
\log(Beta(\mu \mid a_{new}, b_{new})) = log(p(m \mid N, \mu)) + \log \left( p \left( \mu \mid a, b \right) \right)
$$

Substituting our equations from problems 3a and 3b:

$$  
\log(Beta(\mu \mid a_{new}, b_{new})) = \log \left( \mu \right) \cdot m + \log \left( 1 - \mu \right) \cdot (N - m) + (a - 1) \cdot \log(\mu) + (b - 1) \cdot \log(1 - \mu)
$$

Distributing coefficients and gathering like terms:

$$
\log(Beta(\mu \mid a_{new}, b_{new})) = log(\mu) \cdot m + a \cdot log(\mu) - log(\mu) + N \cdot \log(1 - \mu) - m \cdot \log(1 - \mu) + b \cdot \log(1 - \mu) - log(1 - \mu)
$$

Factoring out log terms:

$$
\log(Beta(\mu \mid a_{new}, b_{new})) = \log(\mu)\left( m + a - 1 \right) + \log(1 - \mu) \left( N - m + b - 1 \right)
$$

Removing the constants (because we're unnormalized, I guess?) and rearranging to match the order on the slides:

$$
\log(Beta(\mu \mid a_{new}, b_{new})) = \log(\mu)\left( a + m \right) + \log(1 - \mu) \left( b + (N - m) \right)
$$

So maybe $a_{new} = a + m$ and $b_{new} = b + (N - m)$?
...
We know that the posterior probability is a Beta, and our parameters are $a_{new}$ and $b_{new}$. So that distribution, I'm told, looks like this:

$$
Beta(\mu \mid a_{new}, b_{new}) = \mu^{a_{new} - 1} \times (1 - \mu)^{b_{new} - 1}
$$

Logging both sides so we can derive as the problem asks:

$$ 
f(\mu) = log(Beta(\mu \mid a_{new}, b_{new})) = (a_{new} - 1) \times \log(\mu) + (b_{new} - 1) \times log(1 - \mu)
$$

$$
f'(\mu) = \frac{a_{new} - 1}{\mu} + \frac{b_{new} - 1}{1 - \mu}
$$
...
$$ 
0 = \frac{a_{new} - 1}{\mu} + \frac{b_{new} - 1}{1 - \mu}
$$

$$
\frac{1 - b_{new}}{1 - \mu} = \frac{a_{new} - 1}{\mu}
$$

$$
1 - b_{new} = \frac{a_{new} - 1}{\mu} \times (1 - \mu)
$$

$$
\frac{1 - b_{new}}{a_{new} - 1} = \frac{1 - \mu}{\mu}
$$

I know that the correct formula is $\mu = \frac{a - 1}{a + b - 2}$, so I didn't quite get there. I think the mistake is algebraic, but I'm not sure how to simplify this better.
...
```{r, solution_04c_a}
df_5 <- df_4 %>%
  mutate(posterior_mean = a_new / (a_new + b_new)) %>%
  mutate(posterior_mode = (a_new - 1)/(a_new + b_new - 2)) %>%
  mutate(percentile_05 = qbeta(.05, shape1 = a_new, shape2 = b_new)) %>%
  mutate(percentile_95 = qbeta(.95, shape1 = a_new, shape2= b_new))
df_5
```
...
When the dataset is small, we see that the range of the percentiles (95th percentile - 5th percentile) is quite large, suggesting that we're pretty uncertain about our predicted mean. In contrast, the difference between these percentiles with the large dataset is much smaller, suggesting greater certainty. As a result of the wider quantiles, the posterior mean and mode are further apart in the small dataset and much closer together in the large dataset. In both cases, the informative prior results in greater certainty than the vague prior, though it impacts the small dataset much more (i.e. the range between quantiles for the small dataset is much smaller when using the informed prior).