...
The dataset is roughly balanced, with a difference in observation classes under 3%.
...
```{r, solution_01c}
ctrl_acc <- trainControl(method = "repeatedcv", number = 10, repeats = 2)
metric_acc <- "Accuracy" 
```
...
```{r, solution_01e_03}
dotplot(acc_results, df, metric = metric_acc)
```


Which model is the best? 

Based solely on accuracy, the third model appears the best. Combined with the balanced nature of the dataset, this may suggest a good model. However, if accuracy were the whole story, the assignment would end here, and it doesn't, so on we go.
...
#### worst model by accuracy (8)

```{r}
confusionMatrix(mod_8_acc)
```

#### Second-worst model by accuracy (5)
```{r}
confusionMatrix(mod_5_acc)
```

#### Second-best model by accuracy (2)

```{r}
confusionMatrix(mod_2_acc)
```

#### Best model by accuracy (3)

```{r}
confusionMatrix(mod_3_acc)
```

In general, it seems like false negatives and false positives decrease as the models get better (by accuracy). This makes sense, as accuracy is defined as the ratio of true positives/negatives to total observations, so we'd expect higher accuracy models to have fewer false predictions.
...
```{r, solution_02c_03}
roc_results %>% dotplot(df, metric = metric_roc)
```


Which model is the best?  

According to the ROC results, the third model is best, maximizing area under the ROC curve. Interestingly, the best two and worst two models are the same as the ones identified by accuracy evaluation. This makes me more confident that model 3 is the best.
...
```{r}
roc_results %>% dotplot(df)
```

##### Interpretation

At the default threshold, the models 2 and 3 are tied in average sensitivity (TPR) across all trials. Similarly, model 3 has the highest average specificity and therefore the lowest FPR (since FPR = 1 - specificity).
...
Sensitivity describes the model's ability to correctly predict a positive occurrence when an event actually did occurr (i.e. the proportion of all "positive" events that the model predicted as positive). Since the threshold is very low, a very high proportion of _all_ events are predicted to be positive. This results in a high proportion of the observed events also being predicted positive, and as a result, a high amount of sensitivity.
...
```{r, run_compiile_all_model_preds}
all_model_preds <- compile_all_model_preds(
  mod_1_roc, 
  mod_2_roc, 
  mod_3_roc, 
  mod_4_roc, 
...
Upon a rough visual inspection of the curves, model 3 appears to have the largest AUC. This is consistent with the `dotplot` results above which show model 3 having the highest AUC. Model 8 appears to be the least effective model because it appears to have the lowest AUC. 
...
This shows that I produced 10 bins from 0-1 by 0.1.
...
Bins vary widely in proportion of events and nonevents. For example, the bin with lower bound `0.3` mostly contains non-events, while the bins with lower bounds `0.8` and `0.9` mostly or entirely contain events.
...
My interpretation of this figure is that it suggests a miscalibrated classifier. For example, the first bin contains events with predicted probability less than `0.1`. However, nearly half of the observations in this bin were events. In the `0.3-0.4` bin, we would expect roughly 35% or so of the observations to represent an actual event; empirically, we see that less than 20% of them are.
...
```{r}
my_cal_curve_1 <- test_df_1_b %>%
  select(c("event", "pred_bin", "y")) %>%
  as_tibble() %>%
  mutate(mid_bin = as.numeric(pred_bin) * .1 - .05) %>%
  add_count(pred_bin, name = "pred_bin_count") %>%
  group_by(pred_bin) %>%
  mutate(prop_event = sum(ifelse(y == "event", 1, 0)) / pred_bin_count)

my_cal_curve_1 %>% glimpse
```
...
This appears to communicate similar information to the stacked bar chart we saw above. For example, the first bar in the chart (corresponding to the first and therefore lowest-probability bin) shows a disproportionately high number of predicted events, and we see that reflected here through the dot appearing high above the 45 degree line. As in the bar chart, we see more expected values for the .6, .8, and .9 bins.
...
**Based on your calibration curves, which of the three models appears the most well calibrated? Is it easily obvious which model performs better using this approach? What are the most obvious aspects of performance based on your calibration curves?**  

#### SOLUTION

What do you think?  

Of these, model 3 seems the best calibrated, as most of its proportions align with empirical results. The most interesting finding revealed here is that while the winning model is decent, it incorrectly predicts the 40% bin noticeably. Model 8 seems like it may be overfitted, as it does not generate extremely high or low-probability predictions (suggesting high bias?)
...
Yes, the figure is consistent with the one I generated earlier in the assignment.
...
Yes, the figure is consistent with the one I generated earlier in the assignment (though this one correctly plots the 0-valued bins).