...
```{r, solution_01e_e, eval=FALSE}
### 5th order
set.seed(2001)
mod_high_5 <- train()
```

```{r, solution_01e_f, eval=FALSE}
### 6th order
set.seed(2001)
mod_high_6 <- train()
```

```{r, solution_01e_g, eval=FALSE}
### 7th order
set.seed(2001)
mod_high_7 <- train()
```

```{r, solution_01e_h, eval=FALSE}
### 8th order
set.seed(2001)
mod_high_8 <- train()
```

```{r, solution_01e_i, eval=FALSE}
### 9th order
set.seed(2001)
mod_high_9 <- train()
```
...
```{r, assemble_resampling_high, eval=FALSE}
...
The data are more tightly clustered around a central line, suggesting lower noise.
...
```{r, assemble_resampling_low, eval=FALSE}
...
```{r, solution_02c_b}
dotplot(low_noise_results, metric = 'Rsquared')
```

Is there a best model? Are the results different from the high level noise results?  

It seems relatively clear that the 5th-order polynomial is the best model, having lowest average RMSE and highest (nearly perfect!) correlation by Pearson's R. Both metrics have very low variability, suggesting a highly biased model.
...
```{r, solution_03a}
model_pred_df$obs_class %>% n_distinct()
```
...
```{r, solution_03b}
model_pred_df$pred_prob %>% mean
```

Is the data set balanced?

The data set is roughly balanced, with roughly 45% of cases representing the `non-event` class.
...
model_pred_df <- model_pred_df %>%
  mutate(y = ifelse(obs_class == 1, 1, 0))
```
