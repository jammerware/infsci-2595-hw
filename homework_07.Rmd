...
Based on the figure, the response appears to be linearly related to the input with a negative slope.
...
```{r, solution_01c_c}
lm_logpost(c(-1, -1, -1), my_info = info_01)
```
...
print(laplace_01$mode)
print("Posterior SDs")
print(sqrt(diag(laplace_01$var_matrix)))
```
...
generate_lm_post_samples <- function(mvn_result, length_beta, num_samples)
{
  MASS::mvrnorm(
    ...
    purrr::set_names(nm = c(sprintf("beta_%02d", 0:(length_beta-1)), "varphi")) %>% 
...
##### Probability that beta 1 > 0

```{r}
mean(post_samples_01$beta_01 > 0)
```
The probability that $\beta_1 > 0$ is *`r mean(post_samples_01$beta_01 > 0)``*.
...
##### Do the columns correspond to the number of prediction points?

Yes - the number of columns is 2500, which is the number of posterior samples used for prediction.
...
##### Why consider use the mean trend rather than the predicted response when evaluating the error with the observed response?

We do this because we're focused on how well the model approximates the deterministic portion of the uncertainty (specifically, uncertainty in the estimate of the coefficients, not uncertainty in the unknown standard deviation).
...
##### What are you averaging over when you calculate the mean absolute error?

We're averaging over the difference between what the model predicts and the observed value of `y`. That is, given a posterior sample of coefficients, what is the absolute distance between the value they predict and the observed value?

##### Are the three MAE values the same? If not, why would they be different?

They are different, but this isn't surprising - due to the noise, the model predicts some values of x better than others.
...
```{r, solution_02e}
absE01mat <- post_pred_samples_01 - train_01
...
These results are consistent with the findings from **2b.**
...
The MAE is uncertain because we're only calculating error with respect to the deterministic portion of the model. We're not accounting for the noise in the unknown standard deviation.
...
ggplot() +
  ...
  geom_point(data = test_02, mapping = aes(x = x, y = y), color = "blue", shape = "square")
```
...
The training matrices have *150* rows, while the test matrices have *50*.
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  init_beta <- rnorm( 
    n = ncol(Xtrain),
    mean = my_settings$mu_beta,
    sd = my_settings$tau_beta)
  
  init_varphi <- log(rexp(1, rate = my_settings$sigma_rate))
  
  my_laplace(
    start_guess = c(init_beta, init_varphi),
    logpost_func = logpost_func,
    my_settings
  )
}
```
...
calc_mae_from_laplace <- function(mvn_result, Xtrain, Xtest, y_train, y_test, num_samples)
{
  ...
  mae_train_df %>%
    ...
    mutate(J = length(mvn_result$mode) - 1)
}
```
...
```{r, solution_03d_b}
set.seed(52133)
all_spline_mae_results <- purrr::pmap_dfr(
  list(
    all_spline_models,
    spline_matrices$design_matrix,
...
Based on these visualizations, it seems clear that spline models with order greater than 8 or so overfit the training data.
...
```{r, solution_03f_b}
exp_evidence <- exp(spline_evidence)
spline_weights <- exp_evidence / sum(exp_evidence)
...
Based on this visualization, the spline with **7** degrees of freedom is most probable.
...
Yes, both methods seem to suggest that a relatively low-order spline model is performs best/is most likely. 

##### How do the assessments from the data split comparison compare to the evidence-based assessment?

The metrics seem close, but the data split suggested a suggested a slightly-higher-order model (maybe less than 10 degrees of freedom) might be better. This could simply be due to the randomness of sampling the train/test data.
