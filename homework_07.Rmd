...
my_laplace <- function(start_guess, logpost_func, ...)
{
  ...
  int <- p / 2 * log(2 * pi) + 0.5 * log(det(post_var_matrix)) + logpost_func(mode, info_01)
...
```{r, solution_01d_b}
laplace_01 <- my_laplace(c(0, 0, 0), lm_logpost)
```
...
```
The probability that $\beta_1 > 0$ is *0*.
...
error_01_post_01 <- post_pred_samples_01$Umat[1] - train_01$y[1]

### error of the second posterior sample
error_01_post_02 <- post_pred_samples_01$Umat[2] - train_01$y[2]

### error of the third posterior sample
error_01_post_03 <- post_pred_samples_01$Umat[3] - train_01$y[3]
```

##### Why consider use the mean trend rather than the predicted response when evaluating the error with the observed response?

We do this because we're focused on how well the model approximates the non-stochastic portion of the uncertainty (specifically, uncertainty in the estimate of the coefficients, not uncertainty in the unknown standard deviation).
