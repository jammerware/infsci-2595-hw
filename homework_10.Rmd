...
The two rows in `beta_matrix` correspond to the two unknowns to be learned (the slope and intercept, or weight and bias). The output layer, which is modeled with `alpha_vector`, has three elements because it has a single intercept (bias) and an element for each unknown to learn (the number of rows in the beta matrix).
...
```{r, solution_01i}
prob_01_df %>% 
  mutate(nnet_f = calc_nnet_response(nnet_hidden_2a$H, nnet_hidden_2a$A)) %>% 
...
results_2hidden <- prob_01_df %>% 
  mutate(
    nnet_fa = calc_nnet_response(nnet_hidden_2a$H, nnet_hidden_2a$A),
    nnet_fb = calc_nnet_response(nnet_hidden_2b$H, nnet_hidden_2b$A)
  )
```
...
`h_05` gets coerced to zero much more quickly by the non-linear output for values of X above 1 in comparison to its linear predictor value.
...
results_5hidden <- prob_01_df %>% 
  mutate(
    nnet_fa = calc_nnet_response(nnet_hidden_5a$H, nnet_hidden_5a$A),
    nnet_fb = calc_nnet_response(nnet_hidden_5b$H, nnet_hidden_5b$A)
  )
```
...
my_neuralnet_sse <- function(theta, my_info)
{
  ...
  length_beta_per_unit <- # how many betas are there???????
  total_num_betas <- # how many total betas are there???????
  ...
  a0 <- # output layer bias??????
  aw <- # output layer weights?????
