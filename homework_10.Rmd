...
The two rows in `beta_matrix` correspond to the two unknowns to be learned (the slope and intercept). 

**TODO why does alpha have 3**
...
```{r, solution_01i, eval=FALSE}
prob_01_df %>% 
  mutate(nnet_f = calc_nnet_response( )) %>% 
...
results_2hidden <- prob_01_df %>% 
  mutate(nnet_fa = calc_nnet_response( ),
         nnet_fb = calc_nnet_response( ))
```
