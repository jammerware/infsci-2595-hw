...
In the linear model with the sine wave basis, there are only two unknowns (slope and intercept). In a neural net with 5 hidden units, there are **16** (5 hidden units * 2 unknowns + 5 hidden units + 1).
...
list(
  name = model_names,
  sse = fits %>% purrr::map_dbl(function(x) x$value)
) %>%
  as.tibble() 

```

Based on the training set, model `24a` had the best SSE.
...
assess_nnet_mse <- function(theta, num_hidden, transform_func, X, y)
{
  ...
  sse <- my_neuralnet_sse(theta, info)
...
df_models <- list(
  model = model_names,
  parameters = purrr::map(fits, function(x) x$par),
  num_hidden = c(3, 3, 6, 6, 12, 12, 24, 24)
) %>%
  as.tibble() %>%
  mutate(mse = purrr::map2_dbl(parameters, num_hidden, function (parameters, num_hidden) {
      assess_nnet_mse(
        theta = parameters, 
        num_hidden = num_hidden,
        transform_func = tanh,
...
Based on hold-out set performance, the first 3-unit model performs the best.
...
The tuned model has **5** hidden units with a decay of **0.1**.
...
`size` corresponds to the number of units in the hidden layer. `decay` is a regularization parameter that constrains the size of the coefficients in the hidden layer.
