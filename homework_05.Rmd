...
I completed this assignment on my own. In some cases, I relied heavily on materials posted on Canvas, so some derivations are taken directly from there.
...
**The interval between these two quantiles contains what fraction of the probability mass?**

#### SOLUTION

Some variables for ease of calculation:
...
First, we calculate the quantiles using `qnorm`.

```{r}
qt25_01 <- qnorm(.25, mean = mean_01, sd = sd_01)
qt75_01 <- qnorm(.75, mean = mean_01, sd = sd_01)

print(qt25_01)
print(qt75_01)
```
We can use `pnorm` to find the cumulative density of each quantile and then subtract them. We'd expect this to be 0.5 of the total density by definition.

```{r}
pnorm(qt75_01, mean = mean_01, sd = sd_01) - pnorm(qt25_01, mean = mean_01, sd = sd_01)
```
And indeed it is.
...
I think we can solve this by finding the cumulative density under 117.

```{r}
pnorm(117, mean = mean_01, sd = sd_01)
```

So the prior probability that the mean performance is less than 117 is **`r pnorm(117, mean = mean_01, sd = sd_01)`.**
...
I found this formula in the slides that I think does what I need it to:
...
$$
N = \sigma^2 \left( \frac{1}{\tau^2_N} - \frac{1}{\tau^2_0} \right)
$$

With $\tau_0 = 5$, $\sigma = 5$, and target $\tau_N = 2$, we plug and chug:

$$
N = 5^2 \left( \frac{1}{2^2} - \frac{1}{5^2} \right)
$$

$$
N = 5.25
$$
Thus, we need **5.25** observations (round up to 6, I guess?) to attain a posterior standard deviation of 2. If we want to get to $\tau_N = 0.5$, we can plug that value in and get:

$$
N = 5^2 \left( \frac{1}{0.5^2} - \frac{1}{5^2} \right)
$$

$$
N = 99
$$
Thus, we need **99** observations to attain a posterior standard deviation of 0.5.
...
From the slides, given an infinitely diffuse prior:
...
With $\sigma = 15$ and $\tau_N = 3$:

$$
N \rightarrow \left( \frac{15}{3} \right)
$$

$$N = 25$$
Thus, **25** tests are needed to attain a posterior standard deviation of 3 given an infinitely diffuse prior and a prior standard deviation of 15.
...
$$
\log \left( p \left( \mu \mid \mathbf{x}, \sigma \right) \right) = \log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)
$$

$$
\log \left( p \left( \mu \mid \mathbf{x}, \sigma \right) \right) =  \log \left[ \frac{1}{\sqrt(2 \pi \sigma^2)} \times exp(\frac{1}{2 \sigma^2} \left( (x_n) - \mu \right)^2) \right]
$$

Distribute the log and rewrite square root as a power:

$$
\log(p( x_n \mid \mu, \sigma)) = \sum_{n = 1}^{N} \left( -\frac{1}{2} \log(2\pi) - \frac{1}{2} \log(\sigma^2) - \frac{1}{2 \sigma^2} \left( x_n - \mu \right)^2 \right)
$$

Distributing the summation and dropping non $\mu$ terms:

$$
\log(p( x_n \mid \mu, \sigma)) \propto \sum_{n = 1}^{N} \left(  - \frac{1}{2 \sigma^2} \left( x_n - \mu \right)^2 \right)
$$
...
**NOTE:** (I'm writing these notes to myself as I go from the video posted on canvas, which are in some cases quotes from the video. Not claiming these ideas as my own.)

$$
\frac{d}{d\mu} \log(p( x_n \mid \mu, \sigma)) \propto \frac{d}{d\mu} \sum_{n = 1}^{N} \left(  - \frac{1}{2 \sigma^2} \left( x_n - \mu \right)^2 \right)
$$

*"Since derivatives are linear operations, [we can] place the derivative inside the summation series..."*

$$
\frac{d}{d\mu} \log(p( x_n \mid \mu, \sigma)) \propto \sum_{n = 1}^{N} \left( \frac{d}{d\mu} \left(  - \frac{1}{2 \sigma^2} \left( x_n - \mu \right)^2 \right) \right)
$$

To ease my typing fingers, I'm just going to focus on deriving the expression inside the summation.

$$
- \frac{1}{2 \sigma^2} \frac{d}{d\mu} \left( (x_n - \mu)^2 \right) = \frac{1}{\sigma^2} \left(x_n - \mu \right)
$$

$$
\frac{d}{d\mu} \log(p( x_n \mid \mu, \sigma)) \propto \sum_{n = 1}^{N} \left( \frac{1}{\sigma^2} \left(x_n - \mu \right) \right) = \frac{1}{\sigma^2} \left( \sum_{n=1}^{N}{x_n} - \sum_{n=1}^{N} \mu \right)
$$
The derivative sum from n=1 to N of $\mu$, a constant, is just $\mu \times N$. We can utilize our knowledge of the sample average to identify the sum from $n=1$ to $N$ of $x_n$ as $\bar{x} \times N$.

$$
\frac{d}{d\mu} \log(p( x_n \mid \mu, \sigma)) = \frac{1}{\sigma ^2} \left( \bar{x}N - \mu N \right)
$$

Simplify:

$$
\frac{d}{d\mu} \log(p( x_n \mid \mu, \sigma)) = \frac{N}{\sigma^2} \left( \bar{x} - \mu \right)
$$
...
$$
0 = \frac{N}{\sigma^2} \left( \bar{x} - \mu_{MAP} \right)
$$

Divide both sides by the $\frac{N}{\sigma^2} and add $\mu_{MAP}$ to obtain:
...
The MAP is equivalent to the maximum likelihood estimate and the sample average. Because we're using a Gaussian distribution, we know that the posterior distribution is also a Gaussian. Thus, the posterior mode is the same as the MLE, because in the Gaussian distribution the mean, median, and mode are all equal.
...
$$
\log \left[ p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto \log \left[ \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right] + \log \left[ \mathrm{Exp} \left( \sigma \mid \lambda \right) \right]
$$

Let's work with the exponential distribution term first. Subbing the PDF for the exponential distribution and applying the rules of logs, we get:

$$
\log \left[ \mathrm{Exp} \left( \sigma \mid \lambda \right) \right] = \log \left( \lambda \right) + \log \left( \exp \left( - \lambda \sigma \right) \right) = \log \left( \lambda \right) - \lambda \sigma
$$

Since $\log \left( \lambda \right)$ is not dependent on $\sigma$, we can drop it, leaving us with $- \lambda \sigma$. Subbing this back in above, we get:

$$
\log \left[ p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto \log \left[ \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right] - \lambda \sigma
$$

To deal with the other term, I refer to 2a, in which we had this value for the log of the normal distribution:

$$
\sum_{n = 1}^{N} \left( -\frac{1}{2} \log(2\pi) - \frac{1}{2} \log(\sigma^2) - \frac{1}{2 \sigma^2} \left( x_n - \mu \right)^2 \right)
$$

We can drop the constant term involving $\pi$, but we're left with:

$$
\sum_{n = 1}^{N} \left(- \frac{1}{2} \log(\sigma^2) - \frac{1}{2 \sigma^2} \left( x_n - \mu \right)^2 \right)
$$

We can distribute the summation as we substitute back in to the greater expression:

$$
\log \left[ p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto - \frac{1}{2} \sum_{n=1}^{N}  \log \left( \sigma^2 \right) - \frac{1}{2\sigma^2} \sum_{n=1}^{N}  \left( x_n - \mu \right)^2 - \lambda \sigma
$$

We can also simplify the first term because it doesn't depend on individual values of N and is thus constant. The sum from $n=1$ to $N$ of $log(\sigma^2)$ is simply $N \log(\sigma^2)$, so we arrive finally at:

$$
\log \left[ p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto - \frac{1}{2} N \log(\sigma^2) - \frac{1}{2\sigma^2} \sum_{n=1}^{N}  \left( x_n - \mu \right)^2 - \lambda \sigma
$$
...
Rearrange to find a useful substitution piece:
...
$$
\log \left[ p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto - \frac{1}{2} N \log(\sigma^2) - \frac{vN}{2\sigma^2} - \lambda \sigma
$$

The substitution of $v$ eliminates the dependence of the un-normalized log-posterior on $x_n$. Thus, the information in $v$ encodes the relevant info from $x_n$, which makes sense since it is the sufficient statistic.
...
{
  term_1 <- -0.5 * (N * sigma^2)
  term_2 <- (v * N)  / (2 * sigma^2)
  term_3 <- lambda * sigma
...
It has a single mode (apparently a little under 1.5), but the parabola is a bit skewed.
...
Since the link function is the log, the inverse link function must be $exp()$. We start with:
...
$$
\frac{d\sigma}{d\varphi} = \frac{d}{d\varphi} \exp \left( \varphi \right) = \exp(\varphi)
$$

Taking the log of the derivative:

$$
\log \left( \frac{d\sigma}{d\varphi} \right) = \log \left( \exp \left( \varphi \right) \right) = \varphi
$$
...
tibble::tibble(
  ...
  mutate(log_post = log_post_varphi_unnorm(varphi, N, v, lambda = 0.75)) %>%
...
Yes, the log posterior appears more parabolic here.
...
According to the documentation, we can ask `optim()` to maximize instead of minimize by setting `fnscale` in the `control` list. `gr` should be set to NULL since it isn't used for the `"Brent"` method. It also defaults to NULL.
...
print(varphi_opt_b)
```
Here, the posterior mode is equal to when we started with a guess of 1.35. From our discussion in class, this is probably because the function we're optimizing is unimodal.
...
Since the vertical line seems to intersect the parabola at its maximum, we can conclude that `optim()` has found the MAP of the log-transformed $\varphi$.