...
- Can we tune purely linear frequentist models?
- Is it okay to replace my top performing model from iiA because one of the required models here is identical?
- Are the warnings I have below with model 2 my fault, and if so, what should I do?
...
my_supertrain <- function(formula, data, method = "lm", parallelize = TRUE, ...) {
  ...
  out <- tryCatch(
    ...
    warning = function(cond) {
      message(paste("Warning in supertrain:", cond))
      return(NULL)
    },
    finally = {
...
**QUESTION:** Why does this model interact region with customer and the other inputs?

```{r}
model2 <- my_supertrain(
  response_log ~ (region + customer) + (.^2),
  data = df
)
